...
class Laporan_pegawai_penempatan_kerja extends BaseController
{
    /*
    ...
    public function index()
    {
        if (session('logged_in') !== null && session('stts') === 'administrator') {
            ...
            if (session('id_satuan_kerja') === 'Semua') {
                $set_lap1['lokasi_kerja'] = session('id_lokasi_kerja');
                ...
                $set_lap2['id_satuan_kerja'] = session('id_satuan_kerja');
                $set_lap2['lokasi_kerja']    = session('id_lokasi_kerja');
