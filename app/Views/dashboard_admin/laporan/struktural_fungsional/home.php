  ...
  <div class="well">
	<div class="navbar navbar-inverse">
	  <div class="navbar-inner">
		...
		<?= form_open('laporan_pegawai_struktural_fungsional/set', 'class="navbar-form pull-right"')
        ?>
			...
			<option value="">- Status Jabatan Pegawai -</option>
			  	<?php
                    foreach ($mst_status_jabatan->result_array() as $msk) {
                        if (session('id_status_jabatan') === $msk['id_status_jabatan']) {
                            ?>
			  		<option value="<?= $msk['id_status_jabatan']; ?>" selected="selected"><?= $msk['nama_jabatan']; ?></option>
			  	<?php
                        } else {
                            ?>
			  		<option value="<?= $msk['id_status_jabatan']; ?>"><?= $msk['nama_jabatan']; ?></option>
			  	<?php
                        }
                    }
                ?>
			...
			<option value="">- Eselon -</option>
			  	<?php
                    foreach ($mst_eselon->result_array() as $msk) {
                        if (session('id_eselon') === $msk['id_eselon']) {
                            ?>
			  		<option value="<?= $msk['id_eselon']; ?>" selected="selected"><?= $msk['nama_eselon']; ?></option>
			  	<?php
                        } else {
                            ?>
			  		<option value="<?= $msk['id_eselon']; ?>"><?= $msk['nama_eselon']; ?></option>
			  	<?php
                        }
                    }
                ?>
		...
		<div class="span4 pull-right">
  		<a class="btn" href="<?= base_url(); ?>laporan_pegawai_struktural_fungsional/export"><i class="icon-ok-circle"></i> Export ke Excell</a>
